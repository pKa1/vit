...
const ctaItems = [
  ...
  "Полный цикл — от эскиза до сдачи объекта",
  ...
  return (
    <section className="bg-white py-16 sm:py-32">
      <div className="mx-auto w-full px-6 sm:px-8 lg:px-12">
        <div className="flex flex-col items-start gap-16 lg:flex-row lg:items-start lg:gap-12">
          {/* Left — heading + logo carousel */}
          <div className="flex w-full flex-col items-start space-y-8 lg:w-1/2">
          ...
          <div className="w-full lg:w-1/2">
            <div className="flex flex-col items-start justify-between gap-8 rounded-2xl border border-zinc-200 bg-muted/70 px-8 py-10 shadow-sm md:flex-row lg:px-12 lg:py-14">
              <div className="md:w-1/2">
                <h4 className="mb-2 text-2xl font-medium tracking-tight sm:text-3xl md:text-4xl">
                  Начните проект сегодня
                </h4>
                <p className="text-muted-foreground mt-4 text-sm leading-relaxed md:text-base">
                  Оставьте заявку — обсудим вашу идею и предложим оптимальное архитектурное решение.
                </p>
                <Button className="mt-6" size="lg" asChild>
                  <a href="#contacts">
                    Оставить заявку <ArrowRight className="ml-2 size-4" />
                  </a>
                </Button>
              </div>
              <div className="md:w-1/2">
                <ul className="flex flex-col space-y-3 text-sm font-medium">
                  {ctaItems.map((item, idx) => (
                    <li className="flex items-center" key={idx}>
                      <Check className="mr-3 size-4 flex-shrink-0 text-primary" />
                      {item}
                    </li>
                  ))}
                </ul>
